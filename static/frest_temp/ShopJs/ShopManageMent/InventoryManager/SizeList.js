...
$(function () {
    ...
    if (dt_scrollable_table.length) {
        var requestgetdataurl = $("#SizeManagerUrl").val();
...

                {
                    ...
                    render: function (data, type, full, meta) {
                        ...
                        return (
                            ...
                            `<button  type="button"  class="btn btn-sm btn-icon dropdown-toggle hide-arrow"  data-bs-toggle="modal" data-bs-target="#EditSizeModal_${full['id']}"><i class="bx bx-message-edit bx-tada" ></i></button>`
                        );
                    }
                },
...
function DeleteCategory(cat_id) {
    Swal.fire({
        ...
        if (result.value) {
            var requesturl = $("#SizeManagerUrl").val();
            $.ajax({
...
$(document).ready(function () {
    $("#CreateSizeForm").on("submit", function (e) {
        BtnStyle(true,"CreateSizeBtn")
        e.preventDefault();
...
function EditSize(category_id){
    BtnStyle(true,`EditSizeBtn_${category_id}`)
    var requestUrl = $(`#EditSizeForm_${category_id}`).attr("action");
    var formData = $(`#EditSizeForm_${category_id}`).serialize();
    var csrfToken = $(`#EditSizeForm_${category_id}`).find("input[name='csrfmiddlewaretoken']")
    $.ajax({
        url: requestUrl,
        method:"PUT",
        headers: {
