...
<div class="dz-details">
  ...
  if (dropzoneMulti) {
    ...
    myDropzoneMulti.on("success", function (file, response) {
      ...
      <img src="${response['url']}"  alt="" width="120px" height="120px">
  </a>
      <br>
      <br>
      <button class="btn rounded-pill btn-icon btn-danger" type="button" onclick="DeleteImageFactor(${response['id']})" ><i class="fas fa-trash trash-icon"></i></button>
 
      
      `
      $("#image_factor").empty();
      $("#image_factor").append(img_html);
  })
    myDropzoneMulti.on('removedfile', function (file) {
      ...
      $.ajax({
        type: 'DELETE',
        url: `/InstallMent/ChangeFactorImage/${$("#Factor_obj_id").val()}`,
        headers: {
...
function persianToEnglishDigits(input) {
...
$(document).ready(function () {
  $("#select_costumerrr").on("change", function () {
...
(function () {
  ...
  if (invoiceDateList) {
    invoiceDateList.forEach(function (invoiceDateEl) {
      var date = $("#factor_query_date").val()
      var englishDate = persianToEnglishDigits(date);
...
$(function () {
  ...
  function make_total_price(price, qnty) {
      ...
      $(price).each(function (index) {
          ...
          var quantityValue = Number($(qnty[index]).val());
  
          total_price += priceValue * quantityValue;
          total_num += quantityValue;
      });
  
      
      var separatedNumber = separateNumber(total_price);
...
$(document).on('click', '#CreateFactorBtn', function () {
  ...
  Swal.fire({
    ...
    if (result.value) {
      ...
      var select_costumerrr = $("#select_costumerrr").val();
      ...
      const main_data = {
        ...
        select_costumer: select_costumerrr,
...
$(document).ready(function() {
  var price_vaheds = $(".span_price_vahed")
  price_vaheds.each(function(){
      var number = parseInt(Number($(this).text()), 10);

      var separatedNumber = separateNumber(number);
      $(this).text(` ${separatedNumber}`)
  })
  var price_kol = $(".price")
  price_kol.each(function(){
      var number = parseInt(Number($(this).text()), 10);

      var separatedNumber = separateNumber(number);
      $(this).text(`  ${separatedNumber} تومان`)
  })
  var joze = $(".joze")
  var total = $(".total")

  var number_joz = parseInt(Number(joze.text()), 10);

  var price_joz = separateNumber(number_joz);
  joze.text(`  ${price_joz} تومان`)

  var number_kol = parseInt(Number(total.text()), 10);

  var price_kol = separateNumber(number_kol);
  total.text(`  ${price_kol} تومان`)
})
function DeleteImageFactor(factor_id){
      var parentDiv = $("#dropzone-multii");
      ...
      $.ajax({
        type: 'DELETE',
        url: `/InstallMent/ChangeFactorImage/${factor_id}`,
        headers: {
          // افزودن توکن CSRF به هدر درخواست
          "X-CSRFToken": childInput.val()
        },
